...
describe('PrescriptionListPage', () => {
  ...
  it('renders empty state when no prescriptions exist', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/No prescriptions recorded yet/i)).toBeInTheDocument();
    });
  });
...
describe('PrescriptionCreatePage', () => {
  ...
  it('validates required fields and blocks submission when invalid', async () => {
    ...
    expect(prescriptionApi.createPrescription).not.toHaveBeenCalled();
  });

  it('submits valid prescription draft and navigates', async () => {
    prescriptionApi.createPrescription.mockResolvedValue({ id: 99 });
      ...
      <MemoryRouter initialEntries={['/prescriptions/new']}>
        <AuthProvider>
          <Routes>
            ...
            <Route path="/prescriptions/:id" element={<div>Detail Page 99</div>} />
          </Routes>
        </AuthProvider>
      </MemoryRouter>
    );

    fireEvent.change(screen.getByLabelText(/Patient ID/i), { target: { value: '1' } });
    fireEvent.change(screen.getByLabelText(/Prescribing Dentist ID/i), { target: { value: '2' } });
    fireEvent.change(screen.getByLabelText(/Medicine Name/i), { target: { value: 'Amoxicillin' } });
    fireEvent.change(screen.getByLabelText(/Dosage/i), { target: { value: '500mg' } });
    fireEvent.change(screen.getByLabelText(/Frequency/i), { target: { value: '3x daily' } });
    fireEvent.change(screen.getByLabelText(/Duration/i), { target: { value: '7 days' } });
    fireEvent.change(screen.getByLabelText(/Quantity/i), { target: { value: '21' } });
      ...
      expect(prescriptionApi.createPrescription).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          items: expect.arrayContaining([
            expect.objectContaining({ medicineName: 'Amoxicillin', quantity: 21 })
          ])
        })
      );
      expect(screen.getByText('Detail Page 99')).toBeInTheDocument();
    });
  });
});
...
describe('PrescriptionDetailPage', () => {
  ...
  it('renders DRAFT prescription with Edit, Finalize, and Cancel actions', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Prescription #10/i)).toBeInTheDocument();
      ...
      expect(screen.getByRole('button', { name: /Finalize Prescription/i })).toBeInTheDocument();
    });
  });
  ...
  it('allows finalization when confirmed by authorized dentist', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Finalize Prescription/i }));

    // Modal appears
    expect(screen.getByText(/Finalize Prescription #10\?/i)).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: /Yes, Finalize Prescription/i }));

    await waitFor(() => {
      expect(prescriptionApi.finalizePrescription).toHaveBeenCalledWith('10', '2');
      expect(screen.getByText(/Prescription has been finalized/i)).toBeInTheDocument();
    });
  });
  ...
  it('locks finalized prescription into read-only mode and hides edit button', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Prescription #15/i)).toBeInTheDocument();
...
describe('PrescriptionEditPage', () => {
  ...
  it('blocks editing if prescription is already FINALIZED', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/prescriptions/20/edit']}>
        <Routes>
    ...
    await waitFor(() => {
      expect(screen.getByText(/Cannot Edit Non-Draft Prescription/i)).toBeInTheDocument();
