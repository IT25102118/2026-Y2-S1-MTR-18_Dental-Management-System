...
export default function PrescriptionCreatePage() {
  ...
  const validate = () => {
    ...
    } else if (isNaN(Number(formData.patientId)) || Number(formData.patientId) <= 0) {
    ...
    } else if (isNaN(Number(formData.dentistId)) || Number(formData.dentistId) <= 0) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(`/prescriptions/${created.id}`, {
        state: { message: 'Prescription draft created successfully!' }
      });
    } catch (err) {
      if (err.fieldErrors && Object.keys(err.fieldErrors).length > 0) {
        setErrors((prev) => ({ ...prev, ...err.fieldErrors }));
      }
      setApiError(err.message || 'Failed to create prescription. Please verify the patient and dentist IDs.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="prescription-container">
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div className="prescription-card">
          <h2 style={{ fontSize: '1.25rem', marginTop: 0, marginBottom: '1.25rem', color: '#0f172a' }}>
            Clinical Details
          </h2>
          ...
          <div className="grid-2">
            <div className="form-group">
              ...
              <span className="form-help">Must reference a registered user with PATIENT role.</span>
            </div>

            <div className="form-group">
              <label htmlFor="dentistId">
                Prescribing Dentist ID <span style={{ color: '#dc2626' }}>*</span>
              </label>
              <input
                id="dentistId"
                name="dentistId"
                ...
                required
              />
              {errors.dentistId && <span className="field-error">{errors.dentistId}</span>}
              {user?.role === 'DENTIST' ? (
                <span className="form-help" style={{ color: '#16a34a' }}>
                  Auto-populated with your dentist ID ({user.firstName} {user.lastName}).
                </span>
              ) : (
                <span className="form-help">Must reference a registered user with DENTIST role.</span>
              )}
            </div>
          </div>

          <div className="form-group">
            <label htmlFor="notes">Clinical Notes / Diagnosis Summary (Optional)</label>
            <textarea
              id="notes"
              name="notes"
              rows={3}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1.5rem' }}>
          <Link to="/prescriptions" className="btn btn-secondary" disabled={submitting}>
            Cancel
          </Link>
          <button type="submit" className="btn btn-primary" id="btn-save-draft" disabled={submitting}>
